...
extension UIImage {
  static func svgOperationmodemop(size: CGSize = CGSize(width: 24.0, height: 24.0)) -> UIImage {
    let f = UIGraphicsImageRendererFormat.preferred()
    f.opaque = false
    let scale = CGSize(width: size.width / 24.0, height: size.height / 24.0)
    return UIGraphicsImageRenderer(size: size, format: f).image {
      drawOperationmodemop(in: $0.cgContext, scale: scale)
    }
  }

  private static func drawOperationmodemop(in ctx: CGContext, scale: CGSize) {
