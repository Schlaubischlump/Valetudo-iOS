...
extension UIImage {
  static func svgOperationmodevacuum(size: CGSize = CGSize(width: 24.0, height: 24.0)) -> UIImage {
    let f = UIGraphicsImageRendererFormat.preferred()
    f.opaque = false
    let scale = CGSize(width: size.width / 24.0, height: size.height / 24.0)
    return UIGraphicsImageRenderer(size: size, format: f).image {
      drawOperationmodevacuum(in: $0.cgContext, scale: scale)
    }
  }

  private static func drawOperationmodevacuum(in ctx: CGContext, scale: CGSize) {
