...
@MainActor
final class VTVirtualRestrictionManagementViewController: VTMapViewController {
    /// Robot capabilities that determine which restriction tools are available.
    private let capabilities: Set<VTCapability>
    /// Tracks whether local overlays differ from the last server-authored restriction set.
    private var hasLocalChanges = false
    /// Restriction zone kinds reported by the backend for the current robot.
    private var supportedRestrictedZoneTypes: Set<VTVirtualRestrictionsZoneType> = []

    /// Editable restriction overlays currently displayed on top of the map.
    private var restrictionOverlays: [VTMapOverlay] {
        mapView?.transientOverlays ?? []
    }

    /// Identifier of the currently selected restriction overlay, if any.
    ...
    override var toolbarActionDefinitions: [ToolbarActionDefinition] {
        [
            ...
            ToolbarActionDefinition.separator,
            ToolbarActionDefinition(
                title: "No-Mop",
                image: .noMop,
                handler: { [weak self] in
    ...
    /// Removes live entities unrelated to restriction editing before rendering the map.
    ...
    private func currentVirtualRestrictions() -> VTVirtualRestrictions? {
        ...
        let restrictedZones = restrictionOverlays.compactMap { overlay -> VTRestrictionsZonePayload? in
            guard let zone = overlay as? VTResizableRectangularMapOverlay else { return nil }
